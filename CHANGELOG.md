# Changelog

CCFOLIA Manager の主な変更履歴です。

形式は [Keep a Changelog](https://keepachangelog.com/ja/1.1.0/) を参考にしています。

## [2.0.1] - 2026-09-19

### Security

- CCFOLIAルームからCharacterを取り込む処理を、現在ログイン中のユーザーが所有しているキャラクターだけを対象とするよう変更
- CCFOLIAの現在ユーザーIDをルーム内メンバー情報と照合してからCharacter取り込みに使用
- Firestoreへの問い合わせ段階で `owner == 現在ユーザー` の条件を適用し、他の参加者が所有するキャラクターを取得対象から除外
- 取得したキャラクターをローカルへ保存する前にもowner一致を再確認
- 権限確認のために使用した一時的な診断コードを公開版ブラウザ拡張機能から除去

### Changed

- Characterの取り込み操作を「CCFOLIAから取り込む」から「自分のキャラを取り込む」へ変更
- ブラウザ拡張機能のバージョンを `0.7.1` へ更新

### Notes

- v2.0.0では、Character取り込み時の取得範囲が必要以上に広い実装になっていたため、v2.0.1以降への更新を推奨します
- Images / BGMの取り込み処理はアカウント単位のデータを対象としており、今回のCharacter取り込み修正とは別の処理です

## [2.0.0] - 2026-09-18

### Added

- CCFOLIA ManagerとしてCharacter / Images / BGMを統合管理する構成を追加
- デスクトップアプリにImages管理画面を追加
- デスクトップアプリにBGM管理画面を追加
- ブラウザパネルにCharacter / Images / BGMの切り替えを追加
- ローカル画像ライブラリを追加
- ブラウザからCCFOLIA背景へ画像を適用する機能を追加
- CCFOLIA上の既存画像データをCCFOLIA Managerへ取り込む機能を追加
- 大量の画像を扱うためのページ表示を追加
- ローカルBGMライブラリを追加
- BGMの音量 / ループ / 種別 / タグ / グループ管理を追加
- ブラウザからCCFOLIAへBGMをアップロード・登録する機能を追加
- 現在のCCFOLIAルームのBGMをブラウザパネルから切り替える機能を追加
- CCFOLIA上の既存BGM情報をCCFOLIA Managerへ取り込む機能を追加
- CCFOLIAルーム上の既存キャラクターをCCFOLIA Managerへ取り込む機能を追加
- タスクトレイ運用を追加
- 個人用スナップショットと公開用クリーンビルドを分けるビルド構成を追加

### Changed

- 製品名を `CCFOLIA Character Manager` から `CCFOLIA Manager` へ変更
- Character単体中心の構成から、Character / Images / BGMを扱う統合Managerへ再構成
- ブラウザ拡張機能のパネルを統合Manager向けに拡張
- 公開配布物の名称を `CCFOLIAManager_v2.0.0_Windows_x64_by_L4zz.zip` 形式へ変更
- 大規模な画像ライブラリでもブラウザパネルの初期表示が重くなりにくいよう読み込み方式を改善

### Character

- 1.xで提供していたキャラクター作成・編集・検索・タグ・グループ・Quick Memo・ブラウザ編集・送信・一括操作・ゴミ箱・バックアップ等の基本機能を継続
- キャラクター管理自体の基本操作は1.xから大きく変更していません

### Security

- CCFOLIA連携時に一時的に利用する認証情報をローカルSQLite等へ永続保存しない構成を採用
- ローカルbridgeのHost / Origin / 専用クライアントヘッダー等の検証を継続

### Notes

- CCFOLIAの一部公開されていない内部仕様に依存する機能があります
- CCFOLIA側の仕様変更により、一部連携機能が一時的に動作しなくなる場合があります
- Windows Authenticode署名は未実施
- ブラウザ拡張機能はデベロッパーモードで読み込む方式です

## [1.0.1] - 2026-09-16

### Added

- Windowsアプリ本体から Quick Memo / 臨時メモを閲覧・編集できるようにしました
- タグが1つも設定されていないキャラクターを抽出する「タグ無し」フィルタを追加しました
- ブラウザ上のCCMパネルにも「タグ無し」検索を追加しました

### Changed

- グループツリーの初期状態を折りたたみに変更しました
- キャラクターが所属しているグループや現在選択中のグループについて、必要な親階層のみ自動展開するようにしました
- 多数のグループが存在する場合の一覧性・選択操作を改善しました

## [1.0.0] - 2026-09-16

### Added

- Windows用ローカルキャラクター管理アプリ
- SQLiteによるキャラクターデータ管理
- キーワード検索 / 複数タグAND検索
- 階層グループ管理
- キャラクター複製
- ゴミ箱 / 復元 / 完全削除 / ゴミ箱一括完全削除
- バックアップ / 復元
- CCFOLIA送信
- Brave / Chromium系ブラウザ拡張機能
- ローカルbridge
- ブラウザサイドパネル
- Quick Memo
- ブラウザ内キャラクター編集
- 画像登録 / 並び替え / メイン画像選択 / 削除
- 画像ドラッグ＆ドロップ
- 一括送信 / 一括タグ追加 / 一括グループ追加 / 一括削除
- CCFOLIA風のタグカードUI
- パネル外クリックによる一時非表示と状態保持
- bridgeのHost / Origin / 専用ヘッダー検証
- SHA-256チェックサム付き配布パッケージ生成

### Notes

- 初回公開版
- Windows Authenticode署名は未実施
- ブラウザ拡張機能はデベロッパーモードで読み込む方式
